...
function apply3DEffect(x, y) {
  if (heroPhoto) heroPhoto.style.transform = `translate3d(${x * 12}px, ${y * 12}px, 0) scale(1.05)`;
  if (heroContent) heroContent.style.transform = `translate3d(${x * -20}px, ${y * -20}px, 0)`;
  if (heroHalo) heroHalo.style.transform = `translate3d(${x * 30}px, ${y * 30}px, 0)`;
  if (heroMandala) heroMandala.style.transform = `translate3d(${x * -35}px, ${y * -35}px, 0) rotate(${x * 15}deg)`;
  if (flareContainer) flareContainer.style.transform = `translate3d(${x * 40}px, ${y * 40}px, 0)`;
}
...
window.addEventListener('scroll', () => {
  ...
  if (scrolled < window.innerHeight) {
    const y = scrolled / window.innerHeight;
    if (heroPhoto) heroPhoto.style.transform = `translate3d(0, ${scrolled * 0.25}px, 0) scale(${1 + y * 0.04})`;
    if (heroContent) heroContent.style.transform = `translate3d(0, ${scrolled * -0.18}px, 0)`;
    if (heroHalo) heroHalo.style.transform = `translate3d(0, ${scrolled * 0.35}px, 0)`;
  }
});
